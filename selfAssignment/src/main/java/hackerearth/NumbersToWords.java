...
class NumbersToWords {
    ...
    public static String solution(double number) {
      ...
      String fractionalPart = numberString.substring(numberString.indexOf(".")).substring(1);
      int actualNumber = Integer.parseInt(numberString.substring(0, numberString.indexOf(".")));
      return generate(actualNumber).trim()+" euro "+fractionalPart+" cents";
    }
    ...
    private static String generate1To99(int number) {
      ...
      if (number <= 9) {
        ...
        return tenToNinteen[number % 10]+" euro";
      } else {
